...
int tagFloor = 3;
int receive_sequence = 0;
float other_FPPW = 0.0;
int other_floor = 0;
float this_FPPW = 0.0; 
int floor_change = 0;
...
//  Serial.println("rx_packet");
//  Serial.print(rx_packet);
...
//    Serial.println("RxTimeout == true");
  ...
  {  
    ...
    if(receive_sequence == 0){//if hears character signal
      tagFloor = rx_packet[FLOOR_IDX];
      receive_sequence = 1;
      this_FPPW = DW1000.getFirstPathPower();
      Serial.println("this_FPPW:");
      Serial.print(this_FPPW);
      floor_change = 0;
      Serial.println("I am at floor: ");
      Serial.print(tagFloor);
      print_CIR_POWER();
    }
    received = false;

    Serial.println("rx_packet[FLOOR_IDX]:");
    Serial.print(rx_packet[FLOOR_IDX]);
    if(rx_packet[FLOOR_IDX] == tagFloor){
...
//          deviceRespTs[i].deviceID = currentDeviceIndex;      
//          thisRange[i].device_ID = currentDeviceIndex;
//          thisRange[i].RespRxTime_T = rxTS;
          break;
        }
      }
...
//        Serial.println("=================================");
        if(recvd_resp_seq==seq)
        {
          ...
          std::string anchor_locs[] = {"0,0","3600,0","3600,3100","0,3100"};//anchor locations
          std::string ids = anchor_locs[currentDeviceIndex];
          std::string tdoas = "";
...
//                Serial.println(j);
                ...
                ids = ids+";"+anchor_locs[thisRange[i].device_ID];
                tdoas = tdoas+std::to_string(T_AB_minus_rAB+ToA_mat[thisRange[i].device_ID][currentDeviceIndex])+";";
//                Serial.println(buff);
                //print_CIR_POWER();
                break;
              }
...
//          std::string output = ids+"@"+tdoas;
//          char buff[200];
//          sprintf(buff,output.c_str());
//          Serial.println(buff);
        }
        response_counter =0;
      ...
      receiver(0);
      received = false;
      other_FPPW = DW1000.getFirstPathPower();
      Serial.println("other_FPPW:");
      Serial.print(other_FPPW);
      if(other_floor != rx_packet[FLOOR_IDX]){
        floor_change = 0;
      }
      if(other_FPPW > this_FPPW){
        other_floor = rx_packet[FLOOR_IDX];
        floor_change = floor_change+1;
        Serial.println("change floor:");
        Serial.print(floor_change);
      }
      if(floor_change>=5){
        tagFloor = other_floor;
        Serial.println("FLOOR CHANGED!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! Y'ALL");
      }
      Serial.println("other floors:");
      Serial.print(rx_packet[FLOOR_IDX]);
//       Serial.println("CIR:");
//      print_CIR_POWER();
    }
