...
def create_combos(resp_coor,combos):
    ...
    for num in range(3,num_of_resp+1): #3d hyperbolic model requires at least 3 responders for single result
...
def read(locs,noisy_tdoa_list,seed,sampleNum,initNum):
    ...
    for sample in noisy_tdoa_list:
        ...
        tagLoc = np.reshape(np.array(tagLoc),((1,3)))
        print('tag: ',tagLoc[0].tolist())
